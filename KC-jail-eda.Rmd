...
save_kable(nas_2, file = 'images/missing-values-2.png')
```
...
pop_book <- grid.arrange(tab_off1, tab_off2, ncol = 2, 
...
```{r include=FALSE}
ggsave(filename = 'images/pop_book.png', pop_book)
```
...
```{r include=FALSE}
save_kable(time_pop_off, 'images/time_pop_off.png')
```
...
top_crimes <- 
  jail_df %>% 
    ...
    coord_flip() +
    labs(title = "Top 10 RCW Crimes", 
         x = NULL,
         y = NULL) +
    scale_x_discrete(labels = rev(c('Probation Violation', 'Driving under the influence', 
                                'Theft', 'Assault', 'Public Intoxication',
                                'Drug Possession', 'Traffic Offense', 'Trespassing',
                                'Theft 3rd Degree (less than $750)', 
                                'DUI (Seattle Municipal Code)'))) +
    theme_bw() 
...
```{r include=FALSE}
ggsave(filename = 'images/top_crimes.png', top_crimes)
```
...
jail_tokens <- tokens_wordstem(jail_tokens)
...
save_kable(crime_word_cloud, 'images/crime_word_cloud.png')
...
dict <- dictionary(list(property_crime = c("thef", "thft", "forg", "burg", "brg", 
                        ...
                        probation_parole = c("paro", "com","comm", "cc"),
...
```{r include=FALSE}
save_kable(crime_off_perc, 'images/crime_off_perc.png')
```
...
wilcox_models %>% mutate(offense = fct_reorder(offense, estimate, last)) %>% 
                  ggplot(aes(offense, estimate)) +
                         ...
                         labs(title = "Difference of Incarceration Length by Offense Type", 
                              subtitle = "Repeat Offenders Spend More Time in Jail Per Booking",
                              x = NULL,
                              y = "Days") +
                         theme_bw() 
...
```{r include=FALSE}
ggsave(filename = 'images/jail_diff_off.png', jail_diff_off)
```
